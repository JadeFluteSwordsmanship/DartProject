...
def get_index_circle(inside_or_outside):
    if not (inside_or_outside.startswith('out') or inside_or_outside.startswith('in')):
        raise ValueError('Argument should start with in or out.')
...
#%%
#%%
def plot_dartboard(points_input=None, point_marker='x', text_input=None, flag_index=False, ax_input=None):
    ...
    ## circle 
    circle_point_num = 361
    theta = np.arange(circle_point_num)/180.0*np.pi
    for radius in [R1, R2, R3, R4, R5, R]:
        circle_x = radius*np.cos(theta) + temp_x_off
        circle_y = radius*np.sin(theta) + temp_y_off
        ax.plot(circle_x, circle_y, color='black', linewidth=2)
    
    ## line
    theta_off = 90+9
    for temp_i in range(20):
        theta_off = theta_off - 18
        theta = theta_off/180*np.pi
        temp_x1 = R2*np.cos(theta) + temp_x_off
        temp_y1 = R2*np.sin(theta) + temp_y_off
        temp_x2 = R*np.cos(theta) + temp_x_off
        temp_y2 = R*np.sin(theta) + temp_y_off
        ax.plot([temp_x1, temp_x2], [temp_y1, temp_y2], color='black', linewidth=2)
    ...
    for temp_i in range(20):
        theta_off = theta_off - 18
        theta = theta_off/180*np.pi
        temp_x1 = R_temp*np.cos(theta) + temp_x_off
        temp_y1 = R_temp*np.sin(theta) + temp_y_off
        ax.text(temp_x1, temp_y1, str(d[temp_i]), fontsize=15, horizontalalignment='center', verticalalignment='center')
    

    xlim_left = -200  + temp_x_off
    xlim_right = 200  + temp_y_off
    ax.set_xlim(xlim_left,xlim_right)
    ylim_bottom = -200  + temp_x_off
    ylim_top = 200  + temp_y_off
    ax.set_ylim(ylim_bottom,ylim_top)
    
    xticks = np.array([-170, -100, 0, 100, 170]) + temp_x_off
    ax.set_xticks(xticks)    
    #ax.set_xticklabels([-170, -100, 0, 100, 170])
    ax.tick_params(axis='x', labelsize=12)
    
    yticks = np.array([-170, -100, 0, 100, 170]) + temp_y_off
    ax.set_yticks(yticks)
    #ax.set_yticklabels(['-170(0)', '-100(70)', '0(170)', '100(270)', '170(340)'])
    ax.tick_params(axis='y', labelsize=12)

    if flag_index == False:
        ## show coordinate
        ax.set_xlabel('x-coordinate'.format(), fontsize=12)
        ax.set_ylabel('y-coordinate'.format(), fontsize=12)
    else:
        ## show index
        ax.set_xlabel('x-index'.format(), fontsize=12)
        ax.set_ylabel('y-index'.format(), fontsize=12)
            
    if text_input is not None:
        ax.set_title(text_input, fontsize=12)
    if points_input is not None:
        points = np.array(points_input)
        if len(points.shape)==1:
            ax.scatter(points[0], points[1], marker=point_marker)
        else:
            ax.scatter(points[:,0], points[:,1], marker=point_marker)
